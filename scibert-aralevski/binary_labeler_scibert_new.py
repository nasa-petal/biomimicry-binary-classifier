...
def setup_dataset():
    ...
    if (not os.path.exists(filename)):
        ...
        md5 = 'd20b39efcaca5ac62e44f32effdcbb25' #'fa837a88f0c40c513d975104edf3da17'
        gdown.cached_download(url, filename, md5=md5, postprocess=gdown.extractall)

    df = pd.read_csv(filename)
    df = df[['title', 'abstract', 'petalID', 'doi', 'isBiomimicry']]
    df = df[df['isBiomimicry'].notnull()]
    ...
    docs_test = []
    labels_dict = ['Y', 'N']
    isBiomimicry = df["isBiomimicry"].tolist()
    abstract = df["abstract"].tolist()
    title = df["title"].tolist()
    for i in range(len(title)):
        if i < len(title) - 40:
            docs.append(abstract[i])
            labels.append(labels_dict.index(isBiomimicry[i]))
        else:
            docs_test.append(abstract[i])
            labels_test.append(labels_dict.index(isBiomimicry[i]))
